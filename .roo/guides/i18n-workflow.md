...
The project uses a global `MessageSchema` to provide IDE support. When you type `t('...')`, your IDE should suggest valid keys. If it doesn't, ensure you've updated [`src/locales/types.ts`](../../src/locales/types.ts).

Template strings are currently handled via a flat `template.*` namespace. While they are lazy-loaded at runtime, for TypeScript purposes, they are treated as part of the global schema (or as `any` if not explicitly typed in the core schema).
...
- **Missing Translation Keys**: If the UI shows the raw key (e.g., `app.home.title`), ensure it exists in [`src/locales/en.json`](../../src/locales/en.json) and that the spelling is exact. Template strings must be prefixed with `template.`.
- **TypeScript Errors**: If `t()` doesn't autocomplete or flags valid keys as errors, update the `MessageSchema` in [`src/locales/types.ts`](../../src/locales/types.ts) to match the JSON structure.
- **`useI18n must be called in setup()`**: This composable only works inside Vue components. For utility functions or outside of components, use the global instance: `import { i18n } from '@/i18n'; i18n.global.t('key')`.
- **Tests Failing with Missing Keys**: Always use the custom `render` from [`@testHelpers/renderer`](../../src/__testHelpers__/renderer.ts) instead of the base `vitest-browser-vue` one.
