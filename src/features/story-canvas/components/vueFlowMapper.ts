...
export const mapProjectToVueFlowElements = (
  ...
  const [contextNodes, canvasNodes] = partition(
    data.steps.filter((n) => template.stepDefinitions.find((s) => s.id === n.stepId) !== undefined),
    (n) => template.stepDefinitions.find((s) => s.id === n.stepId)?.category === 'context'
  )
  ...
  const sidebarNodes: SidebarNode[] = contextNodes.map((node) => ({
    ...
    label: getValueAtPath(
      strings,
      template.stepDefinitions.find((s) => s.id === node.stepId)?.labelText ?? node.stepId
    ) as string,
  ...
  const mappedNodes = canvasNodes.map((node) => {
    const step = template.stepDefinitions.find((s) => s.id === node.stepId)
    return {
      id: node.id,
      type: step?.editorConfig.format === 'plain' ? 'plainText' : 'richText',
  ...
  const edges: Edge[] = data.connections.map((edge) => {
    return {
      id: edge.id,
      source: edge.source,
      target: edge.target,
