...
describe('FileSystemProvider', () => {
  ...
  describe('requestNewFileHandle', () => {
    ...
    it('throws NOT_SUPPORTED if API is missing', async () => {
      vi.stubGlobal('showSaveFilePicker', undefined)
      await expect(provider.requestNewFileHandle('test.json')).rejects.toThrow(
  ...
  describe('readFile', () => {
    it('reads and parses JSON content', async () => {
      ...
      const result = await provider.readFile(mockHandle)
    ...
    it('throws READ_FAILED on error', async () => {
      ...
      await expect(provider.readFile(mockHandle)).rejects.toThrow(
  ...
  describe('writeFile', () => {
    it('writes JSON string to the file', async () => {
      ...
      await provider.writeFile(mockHandle, mockData)
    ...
    it('throws PERMISSION_DENIED on NotAllowedError', async () => {
      ...
      await expect(provider.writeFile(mockHandle, {})).rejects.toThrow(
  ...
  describe('getFileName', () => {
    it('returns the name property of the handle', () => {
      const mockHandle = { name: 'my-story.json' } as FileSystemFileHandle
      expect(provider.getFileName(mockHandle)).toBe('my-story.json')
    })
  })
})
