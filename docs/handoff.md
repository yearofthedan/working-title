...
Add `"dev.containers.copyGitConfig": false` and `"dev.containers.gitCredentialHelperConfigLocation": "none"` (stop host git creds leaking into the agent container). Pin `latest` features to exact versions; align pnpm to the package.json version. Drop CDP debug port 9222. Remove/pin the root `uv` installer in the Dockerfile if unused. Add `set -euo pipefail` to `onCreate.sh`.
